...
public partial class Web_Pages_PortoQuiz_Home : System.Web.UI.Page
{
    ...
    public void SelectProgram()
    {
        ...
        if (objlist.Count > 0)
        {
            dgprogram.DataSource = objlist;
            dgprogram.DataBind();
            for (int i = 0; i <= dgprogram.Rows.Count - 1; i++)
            {
                string update = (string)Session["update"];
                string delete = (string)Session["delete"];
                if (update == "yes")
                {
                    dgprogram.Rows[i].Cells[2].Enabled = true;
                }
                else
                {
                    dgprogram.Rows[i].Cells[2].Enabled = false;
                }
                if (delete == "yes")
                {
                    dgprogram.Rows[i].Cells[3].Enabled = true;
                }
                else
                {
                    dgprogram.Rows[i].Cells[3].Enabled = false;
                }
            }
        }
    }
    ...
    protected void btndelete_Click(object sender, EventArgs e)
    {
        ...
        BLLProgram objbll = new BLLProgram();
        objbll.Program_id = val;
        int a = objbll.DeleteProgram(objbll);
        SelectProgram();
    }
    protected void btnupdate_Click(object sender, EventArgs e)
    {
        LinkButton obj = (LinkButton)(sender);
        string val = obj.CommandArgument.ToString();
        Session["prg_update"] = obj.CommandArgument.ToString();
        Session["mode"] = "ed";
