...
public partial class Web_Pages_PortoQuiz_Home : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        SelectRight();
        SelectTeachertAssignCourses();
    }

    public void SelectTeachertAssignCourses()
    {
        List<BLLTeacherEnrollment> objlist = new List<BLLTeacherEnrollment>();
        BLLTeacherEnrollment objbll = new BLLTeacherEnrollment();
        objlist = objbll.SelectTeachEnrollment();
        if (objlist.Count > 0)
        {
            dgteacherenroll.DataSource = objlist;
            dgteacherenroll.DataBind();
            for (int i = 0; i <= dgteacherenroll.Rows.Count - 1; i++)
            {
                string update = (string)Session["update"];
                string delete = (string)Session["delete"];
                if (update == "yes")
                {
                    dgteacherenroll.Rows[i].Cells[1].Enabled = true;
                }
                else
                {
                    dgteacherenroll.Rows[i].Cells[1].Enabled = false;
                }
                if (delete == "yes")
                {
                    dgteacherenroll.Rows[i].Cells[2].Enabled = true;
                }
                else
                {
                    dgteacherenroll.Rows[i].Cells[2].Enabled = false;
                }
            }
        }
    }
